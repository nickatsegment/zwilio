// adapted from https://github.com/msinilo/zmachine
package zmachine
...
func (s *ZStack) Dump() {
	log.Tracef("Top = %d, local frame = %d\n", s.top, s.localFrame)

	for i := MAX_STACK - 1; i >= s.top; i-- {
		if i == s.localFrame {
			log.Tracef("0x%X: 0x%X <------ local frame\n", i, s.stack[i])
		} else {
			log.Tracef("0x%X: 0x%X\n", i, s.stack[i])
		}
	}
}
...
func (h *ZHeader) Read(buf []byte) {
	...
	log.Tracef("End of dyn mem: 0x%X\n", h.staticMemAddress)
	log.Tracef("Global vars: 0x%X\n", h.globalVarAddress)
}
...
func (zm *ZMachine) GetObjectProperty(objectIndex uint16, propertyId uint16) uint16 {
	...
	if propData == 0 {
		...
		log.Tracef("Default prop %d = 0x%X\n", propertyId, result)
	} else {
		if numBytes == 1 {
...
func ZCall(zm *ZMachine, args []uint16, numArgs uint16) {
	...
	log.Tracef("Jumping to 0x%X [0x%X]\n", functionAddress, args[0])
...
func ZRead(zm *ZMachine, args []uint16, numArgs uint16) {
	...
	reader := bufio.NewReader(zm.Stdin)
	...
	for i, w := range words {
		...
		log.Tracef("Dictionary address: 0x%X\n", dictionaryAddress)
...
func GenericBranch(zm *ZMachine, conditionSatisfied bool) {
	...
	log.Tracef("Jump address = 0x%X\n", jumpAddress)

	doJump := (conditionSatisfied != branchOnFalse)

	log.Tracef("Do jump: %t\n", doJump)
...
func (zm *ZMachine) InterpretVARInstruction() {
	...
	if twoOp {
		...
		if instruction == 0x4 {
			log.Debugf("InterpretVARInstruction: ZRead")
		}
...
func (zm *ZMachine) InterpretInstruction() {
	...
	log.Tracef("IP: 0x%X - opcode: 0x%X\n", zm.ip, opcode)
...
func (zm *ZMachine) InterpretTillNextZRead() {
	for i := 0; !zm.done; i++ {
		...
		if form == 0x2 {
			...
			if !twoOp {
				...
				if instruction == 0x4 {
					if i != 0 {
						log.Debugf("ret before ZRead (abs IP %x; rel IP %x)", zm.ip, i)
						return
					} else {
						log.Debugf("found ZRead, but rel IP 0 (abs IP %x)", zm.ip)
					}
				}
			}
			zm.InterpretVARInstruction()
...
func (zm *ZMachine) DecodeZString(startOffset uint32) uint32 {
	...
	for i := 0; i < len(zchars); i++ {
		...
		if zc == 0 {
			...
			aindex := zc - 6
			zm.Printf("%c", alphabets[alphabetType][aindex])
		}
...
func (zm *ZMachine) Done() bool {
	return zm.done
}

func (zm *ZMachine) Printf(f string, v ...interface{}) {
	log.Tracef("Printf: %q %v", f, v)
	fmt.Fprintf(zm.Stdout, f, v...)
}

func (zm *ZMachine) IP() uint32 {
	return zm.ip
}
